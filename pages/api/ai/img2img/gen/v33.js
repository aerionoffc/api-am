...
class QwenLorasFast {
  constructor() {
    this.host = "https://prithivmlmods-qwen-image-edit-2511-loras-fast.hf.space";
    this.styles = ["Multiple-Angles", "Photo-to-Anime", "Anime-V2", "Light-Migration", "Upscaler", "Style-Transfer", "Manga-Tone", "Anything2Real", "Fal-Multiple-Angles", "Polaroid-Photo", "Unblur-Anything", "Midnight-Noir-Eyes-Spotlight", "Hyper-Realistic-Portrait", "Ultra-Realistic-Portrait", "Pixar-Inspired-3D", "Noir-Comic-Book", "Any-light", "Studio-DeLight", "Cinematic-FlatLog"];
    this.headers = {
      accept: "*/*",
  ...
  _log(act, msg) {
    console.log(`[QwenLorasFast] [${act}] ${msg}`);
  }
  _hash() {
  ...
  async generate({
    ...
    this._log("Queue", "Starting loras fast pipeline...");
    const sessionHash = rest?.session_hash || this._hash();
    let es = null;
    const chosenStyle = this.styles.includes(style) ? style : "Anime-V2";
    ...
    try {
      ...
      const payload = {
        data: [imageStringArray, prompt || PROMPT.text, chosenStyle, rest?.seed || 0, rest?.enhance ?? true, rest?.steps || 1, rest?.cfg || 4],
        fn_index: rest?.fn_index || 2,
        trigger_id: rest?.trigger_id || 13,
      ...
      this._log("Queue", `Joining queue with style [${chosenStyle}]`);
