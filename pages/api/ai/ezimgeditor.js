...
-----END PUBLIC KEY-----`;
    this._BRAND_NAME = "ezimgeditor.io";
    this._BRAND_ID = 36;
    this._BASE_URL = "https://api.ezimgeditor.io";
    this.originFrom = crypto.createHash("md5").update(this._BRAND_NAME).digest("hex").substring(4, 20);
    this._defaultHeaders = {
      accept: "application/json, text/plain, */*",
      "accept-language": "en-US,en;q=0.9",
      "cache-control": "no-cache",
      origin: "https://ezimgeditor.io",
      referer: "https://ezimgeditor.io/",
  ...
  async _upImg(imageInput) {
    try {
      ...
      const res = await this.client.post("/api/aikit/upload_img", form, {
        headers: headers
      });
      ...
      const fullUrl = relativeUrl?.startsWith("http") ? relativeUrl : `https://temp.ezimgeditor.io/${relativeUrl}`;
    ...
    try {
      ...
      while (!isCompleted && attempts < maxAttempts) {
        ...
        if (status === 0) {
          ...
          finalResult = rawResultPath?.startsWith("http") ? rawResultPath : `https://temp.ezimgeditor.io/${rawResultPath}`;
...
export default async function handler(req, res) {
  ...
  const api = new EzImgEditorClient();
  try {
